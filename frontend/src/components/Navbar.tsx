...
export default function Navbar() {
  ...
  return (
    <nav>
      {/* Desktop Navbar */}
      <div className="hidden md:block fixed inset-y-0 left-0 w-30 bg-highlight-dark text-white flex flex-col items-center justify-between py-4 shadow-lg z-50">
      ...
      {/* Mobile Overlay */}
        <div className={`fixed inset-0 z-50 md:hidden transition-opacity duration-200 ease-out ${
          isMobileMenuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
        }`}>
          <div className={`fixed inset-0 bg-black/50`} onClick={toggleMobileMenu} />

          <div className={`fixed top-0 left-0 bottom-0 w-72 bg-highlight-dark transform transition-transform duration-200 ease-out ${
            isMobileMenuOpen ? 'translate-x-0': '-translate-x-full'
          }`}>
            <h1 className="text-3xl text-accent-purple p-6 font-medium">
              Surge
            </h1>
